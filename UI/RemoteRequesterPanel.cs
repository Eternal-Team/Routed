...
{
	// todo: sorting modes
	public class RemoteRequesterPanel : BaseUIPanel<RemoteRequester>, IItemHandlerUI
	{
		...
		{
			Hooking.Network = Container.Module.Network;
			Recipe.FindRecipes();
		}
		...
		public RemoteRequesterPanel(RemoteRequester module) : base(module)
		{
		...
		public void UpdateGrid()
		{
			...
			for (int i = 0; i < gridSlots.Count; i++)
			{
				...
				if (!Container.Module.Network.ItemCache.ContainsKey(slot.Item.type)) gridSlots.Remove(slot);
			}

			// update
			var remaining = new Dictionary<int, int>();
			foreach (var pair in Container.Module.Network.ItemCache)
			{
